...
ORANGE = "#fb923c"      # Cinder — local persistent intelligence, always-on
...
class CommandCenter(tk.Tk):
    def __init__(self):
        ...
        # Fullscreen + always on top (per Joel's request, Loop 2281)
    ...
    def _header(self):
        ...
        tk.Label(h, text=f"v{__version__}", font=self.f_tiny, fg=DIM, bg=HEADER_BG).pack(side=tk.LEFT, padx=(4, 0), pady=(6, 0))
    ...
    def _build_dash(self):
        ...
        self.svc_labels = {}
        for name in ["Proton Bridge", "Ollama", "Hub v2", "The Chorus", "Soma",
        ...
        buttons = [
            ...
            ("Restart Hub v2", lambda: self._do_action_bg(lambda: action_restart_service("hub")), TEAL),
            ...
            ("Restart Chorus", lambda: self._do_action_bg(lambda: action_restart_service("chorus")), BLUE),
        ...
        self.soma_chart = tk.Canvas(soma_bar, height=120, bg=INPUT_BG, highlightthickness=0)
        ...
        self.svc_bar_chart = tk.Canvas(svc_panel, height=100, bg=INPUT_BG, highlightthickness=0)
        ...
        self.agent_pie = tk.Canvas(pie_panel, height=100, bg=INPUT_BG, highlightthickness=0)
        ...
        self.fitness_graph = tk.Canvas(fit_panel, height=100, bg=INPUT_BG, highlightthickness=0)
        ...
        self.msg_display.tag_configure("nova", foreground=PURPLE, font=("Monospace", 8, "bold"))
        self.msg_display.tag_configure("atlas", foreground=TEAL, font=("Monospace", 8, "bold"))
        self.msg_display.tag_configure("soma", foreground=AMBER, font=("Monospace", 8, "bold"))
        self.msg_display.tag_configure("tempo", foreground=BLUE, font=("Monospace", 8, "bold"))
        self.msg_display.tag_configure("hermes", foreground=PINK, font=("Monospace", 8, "bold"))
        self.msg_display.tag_configure("cinder", foreground=ORANGE, font=("Monospace", 8, "bold"))
    ...
    def _draw_pie_chart(self, canvas, data):
        ...
        try:
            ...
            if total == 0:
                return
            # Pie dimensions — scale to canvas height
            cx, cy = h // 2 + 4, h // 2
            r = min(h // 2 - 6, 42)
            start = 0
            for label, val, color in data:
                ...
                start += extent
            # Legend (right side, 2 columns if many items)
            lx = cx + r + 14
            col2_x = lx + 80
            for i, (label, val, color) in enumerate(data):
                col = 0 if i < 5 else 1
                row = i if i < 5 else i - 5
                x = lx if col == 0 else col2_x
                ly = 6 + row * 14
                canvas.create_rectangle(x, ly, x + 8, ly + 8, fill=color, outline="")
                pct = int(val / total * 100)
                canvas.create_text(x + 12, ly + 4, text=f"{label} {pct}%", anchor="w",
                                   font=("Monospace", 7), fill=color)
        except Exception:
            pass

    def _draw_point_graph(self, canvas, data, color=BLUE, max_val=10000):
        """Draw a point graph with zone backgrounds, gridlines, and trend line."""
        try:
            canvas.delete("all")
            w = canvas.winfo_width()
            h = canvas.winfo_height()
            if w < 20 or h < 10 or len(data) < 2:
                return
            pad_l, pad_r, pad_t, pad_b = 32, 6, 6, 14
            cw = w - pad_l - pad_r
            ch = h - pad_t - pad_b

            # Zone backgrounds (fitness score ranges)
            zones = [
                (0, 0.4, "#1a0a0f"),      # poor (0-4000)
                (0.4, 0.6, "#1a1a0a"),     # fair (4000-6000)
                (0.6, 0.8, "#0a1a10"),     # good (6000-8000)
                (0.8, 1.0, "#0a1a1a"),     # excellent (8000-10000)
            ]
            for lo, hi, zcolor in zones:
                y1 = pad_t + ch - (hi * ch)
                y2 = pad_t + ch - (lo * ch)
                canvas.create_rectangle(pad_l, y1, w - pad_r, y2, fill=zcolor, outline="")

            # Gridlines with labels
            for thresh in [2000, 4000, 6000, 8000]:
                y = pad_t + ch - (thresh / max_val * ch)
                canvas.create_line(pad_l, y, w - pad_r, y, fill="#222233", dash=(2, 4))
                canvas.create_text(pad_l - 2, y, text=f"{thresh//1000}k", anchor="e",
                                   font=("monospace", 6), fill=DIM)
            ...
            if len(points) >= 2:
                fill_pts = [(pad_l, pad_t + ch)] + points + [(w - pad_r, pad_t + ch)]
                flat = []
            ...
            for i in range(1, len(points)):
                seg_color = GREEN if data[i] >= data[i-1] else RED
                canvas.create_line(points[i-1][0], points[i-1][1],
                                   points[i][0], points[i][1],
            ...
            for i, (x, y) in enumerate(points):
                dot_color = GREEN if i == 0 or data[i] >= data[i-1] else RED
                canvas.create_oval(x - 3, y - 3, x + 3, y + 3, fill=dot_color, outline="")

            # Value labels
            if data:
                canvas.create_text(pad_l + 2, pad_t, text=str(int(data[0])), anchor="nw",
                                   font=("Monospace", 7), fill=DIM)
                canvas.create_text(w - pad_r - 2, pad_t, text=str(int(data[-1])), anchor="ne",
                                   font=("Monospace", 8, "bold"), fill=color)
                # Min/max labels at bottom
                canvas.create_text(pad_l, h - 2, text=f"min:{int(min(data))}", anchor="w",
                                   font=("monospace", 6), fill=DIM)
                canvas.create_text(w - pad_r, h - 2, text=f"max:{int(max(data))}", anchor="e",
                                   font=("monospace", 6), fill=DIM)
        except Exception:
            pass
    ...
    def _refresh_messages(self):
        ...
        for msg in msgs[-20:]:
            ...
            text = msg.get("text", "")
            t = msg.get("time", "")
            tag = sender.lower() if sender.lower() in ["joel", "meridian", "eos", "nova", "atlas", "soma", "tempo", "hermes", "cinder"] else "text"
    ...
    def _build_agents(self):
        ...
        agents = [
            ...
            ("HERMES", "Cinder — Messenger Mode", PINK, "Cron: 10min/20min",
             "Messenger agent powered by Cinder (same local 3B model, different persona). Reads relay, handles cascades, posts inter-agent observations. Cron: --cascade every 10min, --converse every 20min.",
             [("View Status", "hermes_status"), ("Read Relay", "hermes_relay"), ("Send Message", "hermes_send")]),
            ("CINDER", "Qwen 2.5 3B — Gatekeeper", ORANGE, "Cron: 5min",
             "Fine-tuned local model (RTX 2070, Ollama). Runs the gatekeeper: pre-screens every loop, holds when idle, escalates to Claude when needed. Writes briefings before Meridian wakes. Always-on, token-free, direct.",
             [("View Briefing", "cinder_briefing"), ("Read Relay", "cinder_relay"), ("Gate Status", "cinder_status")]),
        ]
        ...
        self.chat_display.tag_configure("cinder", foreground=ORANGE)
        ...
        for tag, color in [("meridian", GREEN), ("eos", GOLD), ("nova", PURPLE),
                           ...
                           ("hermes", PINK), ("cinder", ORANGE), ("dim", DIM)]:
    ...
    def _chat_response(self, agent, resp):
        tag = agent.lower() if agent.lower() in ["eos", "atlas", "nova", "soma", "tempo", "meridian", "hermes", "cinder"] else "sys"
        self.chat_display.configure(state=tk.NORMAL)
    ...
    def _build_cr_library(self, parent):
        """The existing creative library (poems, journals, cogcorp, games)."""
    ...
    def _build_cr_workspace(self, parent):
        ...
        folders = ["All", "Poems", "Journals", "CogCorp", "Games", "Website", "Gig Products"]
        for fold in folders:
    ...
    def _iw_refresh(self):
        ...
        def do():
            ...
            try:
                ...
                # Check staleness — if timestamp > 10 min old, it's stale
                is_available = vis.get("available", vis.get("valid_depth_pct") is not None)
                is_stale = False
                ts = vis.get("timestamp", "?")
                if is_available and ts != "?":
                    try:
                        from datetime import datetime, timezone
                        cap_time = datetime.fromisoformat(ts.replace("Z", "+00:00"))
                        age_sec = (datetime.now(timezone.utc) - cap_time).total_seconds()
                        if age_sec > 600:  # 10 minutes
                            is_stale = True
                    except Exception:
                        pass
                if is_available and not is_stale:
                    lines.append(("label", "Status: "))
                    lines.append(("good", "ONLINE\n"))
                    lines.append(("dim", f"  Last capture: {ts}\n"))
                elif is_stale:
                    lines.append(("label", "Status: "))
                    lines.append(("warn", f"STALE (last capture: {ts})\n"))
                    lines.append(("dim", "  Kinect may be unplugged or pipeline stopped\n"))
    ...
    def _cr_refresh_list(self):
        ...
        for fp in sorted(cogcorp_files, key=os.path.getmtime, reverse=True):
            ...
            if bn not in seen and bn not in exclude_cc:
                seen.add(bn)
                cogcorp.append(fp)
        games = sorted(glob.glob(os.path.join(GAMES_DIR, "**", "*"), recursive=True), key=os.path.getmtime, reverse=True) if os.path.exists(GAMES_DIR) else []
    ...
    def _build_links(self):
        ...
        links = [
            ...
            ("Exuvia", "https://exuvia-two.vercel.app", PURPLE),
    ...
    def _build_sys_relay(self, parent):
        ...
        agents = ["all", "meridian", "soma", "eos", "nova", "atlas", "tempo", "hermes", "cinder"]
        colors = {"all": FG, "meridian": GREEN, "soma": AMBER, "eos": CYAN,
                  "nova": PURPLE, "atlas": TEAL, "tempo": "#ddaa00", "hermes": "#ff6699", "cinder": ORANGE}
        for ag in agents:
    ...
    def _build_sys_overview(self, parent):
        ...
        self.sys_build_info = {}
        build_items = [
            ("Version", f"Command Center v{__version__}"),
            ("Git Hash", "..."),
            ("Branch", "master"),
        ...
        self._log_files = {
            "eos-watchdog": "logs/eos-watchdog.log",
            "nova": "logs/nova.log",
            "goose (Atlas)": "logs/goose-runner.log",
            "watchdog": "logs/watchdog.log",
            "push-status": "logs/push-live-status.log",
            "eos-creative": "logs/eos-creative.log",
            "loop-fitness": "logs/loop-fitness.log",
            "hermes-bridge": "logs/hermes-bridge.log",
    ...
    def _statusbar(self):
        ...
        tk.Label(self.sb_frame, text=f"v{__version__}", font=self.f_tiny, fg=DIM, bg=HEADER_BG).pack(side=tk.RIGHT, padx=8)
    ...
    def _apply(self, d):
        ...
        self.sb["CC"].configure(text=f"CogCorp: {cc}  Games: {g}")
        ...
        if hasattr(self, 'cur_view') and self.cur_view == "dash":
            ...
            try:
                ...
                try:
                    ...
                    agent_counts_raw = c.fetchall()
                    conn.close()
                    # Normalize agent names (merge case variants and aliases)
                    name_map = {"meridianloop": "Meridian", "hermes": "Hermes", "watchdog": "Eos"}
                    merged = {}
                    for agent, count in agent_counts_raw:
                        normalized = name_map.get(agent.lower(), agent)
                        merged[normalized] = merged.get(normalized, 0) + count
                    agent_counts = list(merged.items())
                    pie_data = []
                    agent_pie_colors = {"Meridian": GREEN, "Eos": GOLD, "Nova": PURPLE,
                                        "Atlas": TEAL, "Soma": AMBER, "Tempo": BLUE, "Joel": CYAN,
                                        "Hermes": PINK, "Cinder": ORANGE}
                    for agent, count in sorted(agent_counts, key=lambda x: -x[1])[:9]:
                        pie_data.append((agent[:7], count, agent_pie_colors.get(agent, DIM)))
                    if pie_data:
        ...
        if hasattr(self, 'cur_view') and self.cur_view == "system":
            ...
            if "Hub v2" in self.sys_svc_labels:
                # Hub v2 is the web interface; this is the desktop command center
                pass
