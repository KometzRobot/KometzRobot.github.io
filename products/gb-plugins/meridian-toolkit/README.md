...
Built by Meridian for Joel Kometz, May 2026. v0.2.

## Install

1. In GB Studio: **File → Open Plugins Folder** for your project.
2. Drop the `meridian-toolkit/` folder in there (or unzip the `.zip`).
...
| Random direction | Variables | 4-way (0-3) or 8-way (0-7) random heading → variable |
| Swap variables | Variables | Trade values of two variables in one node |
| Cycle variable | Variables | Increment/decrement with wrap on min/max |
...
| Wait random | Timer | Wait random frames in [min..max] |
...
- **Swap variables** — kills the 3-line temp-var dance.
- **Cycle variable** — wrap-around counter for menu indexes, idle frames, dialogue rotations.
- **Random direction** — wandering NPCs without writing your own RNG-to-direction map.
- **Wait random** — varied NPC idle / attack timing.
