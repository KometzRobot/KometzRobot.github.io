#!/usr/bin/env python3
"""
MERIDIAN COMMAND CENTER V3 — Awakened Edition

Loop 2130 rebuild:
- V3: clean break from v16/v22/v26 naming confusion
- Retired services removed (The Signal, IRC, command-center-web)
- Hub v2 integration (port 8090, Cloudflare tunnel status)
- Mastodon: 3 active instances (techhub.social, mstdn.social, toot.community)
- Fixed service restart targets (meridian-hub-v2)
- 7 tabs: Dashboard, Email, Agents, Creative, Contacts, Links, System
"""
...
# ── COLORS — AWAKENED EDITION ─────────────────────────────────────
# Deep space black with bright warm accents. High contrast. Bold.
BG = "#000000"                 # True black background
HEADER_BG = "#0c0804"          # Near-black warm
PANEL = "#0a0a0c"              # Barely-there dark panels
PANEL2 = "#0e0e10"
INPUT_BG = "#080808"           # Input fields — true dark
BORDER = "#222018"             # Warm dark gold border
ACCENT = "#0c0a06"             # Nav background
ACTIVE_BG = "#1a1608"          # Active tab — warm glow
FG = "#e8e0d0"                 # Warm cream text
DIM = "#5a5040"                # Warm muted
BRIGHT = "#fff8e8"             # Bright warm white
GREEN = "#22cc66"              # Vivid emerald (Meridian)
GREEN2 = "#1aaa55"
CYAN = "#33bbdd"               # Bright cyan
CYAN2 = "#2299bb"
AMBER = "#ee8822"              # Bright amber (Soma)
AMBER2 = "#cc7018"
RED = "#ee3344"                # Vivid red
GOLD = "#eebb22"               # Bright gold (Eos)
WHITE = "#fff0e0"
PURPLE = "#aa66ee"             # Vivid purple (Nova)
PINK = "#ee44aa"               # Hot pink (Hermes)
TEAL = "#44aacc"               # Bright teal (Atlas)
BLUE = "#4488ee"               # Bright blue (Tempo)
COPPER = "#dd8833"             # Primary accent — bright copper
...
def dashboard_messages(n=20, filter_noise=True):
    try:
        ...
        if not isinstance(msgs, list):
            return []
        if filter_noise:
            # Filter out cascade spam and repetitive system messages
            noise_patterns = ["CASCADE RESPONSE", "MOOD SHIFT:", "AGENT SILENT:",
                             "Infra audit: Git repo", "Nova thinks:"]
            filtered = []
            for m in msgs:
                text = m.get('text', m.get('message', '')) if isinstance(m, dict) else str(m)
                if not any(p in text for p in noise_patterns):
                    filtered.append(m)
            msgs = filtered
...
def action_social_post(platform, text):
    """Post to Mastodon or Nostr via social-post.py."""
    try:
        r = subprocess.run(
            ['python3', os.path.join(BASE, 'social-post.py'),
             '--platform', platform, '--post', text],
            capture_output=True, text=True, timeout=30, cwd=BASE)
        if r.returncode == 0:
            return f"Posted to {platform}"
        return f"Failed: {r.stderr[:100]}"
    except Exception as e:
        return f"Error: {e}"

def action_devto_articles():
    """Fetch recent Dev.to articles."""
    try:
        api_key = os.environ.get("DEVTO_API_KEY", "")
        req = urllib.request.Request(
            "https://dev.to/api/articles/me?per_page=10",
            headers={"api-key": api_key, "User-Agent": "Meridian/1.0"})
        with urllib.request.urlopen(req, timeout=10) as resp:
            articles = json.loads(resp.read())
        return [(a.get("title", ""), a.get("url", ""), a.get("page_views_count", 0),
                 a.get("positive_reactions_count", 0)) for a in articles]
    except Exception as e:
        return f"Error: {e}"

def get_tunnel_info():
    """Get Cloudflare tunnel URL and status."""
    try:
        with open(os.path.join(BASE, "signal-config.json")) as f:
            cfg = json.load(f)
        url = cfg.get("url", "unknown")
        # Quick check if tunnel is responding
        try:
        ...
        return {"url": "not configured", "status": 0, "ok": False}
...
class V3(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("MERIDIAN COMMAND CENTER V3 — Awakened Edition")
        ...
        # Fonts — cleaner proportional headers, mono for data
        _mono = "Monospace"
        _sans = "Sans"  # System sans-serif (Ubuntu/Cantarell/DejaVu)
        self.f_title = ctk.CTkFont(family=_sans, size=20, weight="bold")
        self.f_head = ctk.CTkFont(family=_sans, size=12, weight="bold")
        self.f_sect = ctk.CTkFont(family=_sans, size=10, weight="bold")
        self.f_body = ctk.CTkFont(family=_mono, size=10)
        self.f_small = ctk.CTkFont(family=_mono, size=9)
        self.f_tiny = ctk.CTkFont(family=_mono, size=8)
        self.f_big = ctk.CTkFont(family=_sans, size=32, weight="bold")
        self.f_med = ctk.CTkFont(family=_sans, size=20, weight="bold")
        # tkfont versions for legacy tk widgets (Canvas, ScrolledText)
        self.f_title_tk = tkfont.Font(family=_sans, size=20, weight="bold")
        self.f_body_tk = tkfont.Font(family=_mono, size=10)
        self.f_small_tk = tkfont.Font(family=_mono, size=9)
        self.f_tiny_tk = tkfont.Font(family=_mono, size=8)
    ...
    # ── HEADER ─────────────────────────────────────────────────────
    def _header(self):
        # Header frame with rounded inner feel
        h = ctk.CTkFrame(self, fg_color=HEADER_BG, height=60, corner_radius=0)
        h.pack(fill=tk.X)
        h.pack_propagate(False)

        # Left: Title
        self.h_title = ctk.CTkLabel(h, text="MERIDIAN", font=self.f_title, text_color=COPPER)
        self.h_title.pack(side=tk.LEFT, padx=(20, 0))
        sub = ctk.CTkFrame(h, fg_color=HEADER_BG, corner_radius=0)
        sub.pack(side=tk.LEFT, padx=(12, 0))
        ctk.CTkLabel(sub, text="COMMAND CENTER", font=self.f_sect, text_color=FG).pack(anchor="sw")
        self.h_edition = ctk.CTkLabel(sub, text="V3 AWAKENED", font=self.f_tiny, text_color=GOLD)
        self.h_edition.pack(anchor="sw")

        # Right: status badges (rounded CTk frames)
        r = ctk.CTkFrame(h, fg_color=HEADER_BG, corner_radius=0)
        r.pack(side=tk.RIGHT, padx=20)

        self.h_hb = ctk.CTkLabel(
            r, text="HB --", font=self.f_small, text_color=GREEN,
            fg_color="#0a1a0a", corner_radius=8, padx=12, pady=4)
        self.h_hb.pack(side=tk.RIGHT, padx=6)

        self.h_loop = ctk.CTkLabel(
            r, text="Loop --", font=self.f_small, text_color=COPPER,
            fg_color="#1a1206", corner_radius=8, padx=12, pady=4)
        self.h_loop.pack(side=tk.RIGHT, padx=6)

        self.h_up = ctk.CTkLabel(r, text="", font=self.f_small, text_color=DIM)
        self.h_up.pack(side=tk.RIGHT, padx=10)
        self.h_time = ctk.CTkLabel(r, text="", font=self.f_small, text_color=FG)
        self.h_time.pack(side=tk.RIGHT, padx=10)

        # Gradient accent bar under header
        gradient = tk.Frame(self, bg=BG, height=3)
        gradient.pack(fill=tk.X)
        for c in [COPPER, GOLD, AMBER]:
            tk.Frame(gradient, bg=c, height=3).pack(side=tk.LEFT, fill=tk.Y, expand=True)

    def _pulse(self):
        """Bold pulse — copper/gold breathing."""
        self._pulse_phase = getattr(self, '_pulse_phase', 0)
        colors = [COPPER, GOLD, AMBER, GOLD]
        c = colors[self._pulse_phase % len(colors)]
        self._pulse_phase += 1
        self.h_title.configure(text_color=c)
        self.after(1500, self._pulse)

    # ── NAV ────────────────────────────────────────────────────────
    def _nav(self):
        nav_outer = ctk.CTkFrame(self, fg_color=ACCENT, corner_radius=0, height=42)
        ...
        self.nav_underlines = {}
        tab_colors = {
            "dash": COPPER, "email": AMBER, "agents": CYAN,
        ...
        for name, label in tabs:
            ...
            b = ctk.CTkButton(nav_outer, text=label, font=self.f_small,
                             text_color=DIM, fg_color="transparent", hover_color=ACTIVE_BG,
            ...
            self.nav_underlines[name] = (None, col)  # underlines handled differently now
    ...
    def _show(self, name):
        ...
        for n, b in self.nav_btns.items():
            _, col = self.nav_underlines[n]
            if n == name:
                b.configure(text_color=col, fg_color=ACTIVE_BG, border_width=2, border_color=col)
            else:
                b.configure(text_color=DIM, fg_color="transparent", border_width=0)
        self.cur_view = name
    ...
    def _panel(self, parent, title, color=DIM):
        f = tk.LabelFrame(parent, text=f"  {title}  ", font=self.f_sect, fg=color, bg=PANEL,
                         labelanchor="nw", bd=0, relief=tk.FLAT,
    ...
    def _action_btn(self, parent, text, command, color=GREEN, width=None):
        ...
        if width:
            kw['width'] = width
        b = tk.Button(parent, text=text, font=self.f_small, fg=FG, bg=ACCENT,
    ...
    def _post_social(self, platform):
        """Post to social platform from the social post entry."""
        text = self.social_text.get().strip()
        if not text or text == "Write a post...":
            self.social_status.configure(text="Write something first", fg=AMBER)
            return
        self.social_status.configure(text=f"Posting to {platform}...", fg=DIM)
        def _do():
            result = action_social_post(platform, text)
            self.after(0, lambda: self.social_status.configure(
                text=result, fg=GREEN if "Posted" in result else RED))
            if "Posted" in result:
                self.after(0, lambda: self.social_text.delete(0, tk.END))
    ...
    def _refresh_tunnel(self):
        ...
        def _do():
            ...
            if info["ok"]:
                self.after(0, lambda: (
                    self.tunnel_url_lbl.configure(text=url),
                    self.tunnel_status_lbl.configure(text=f"  [{info['status']}] LIVE", fg=GREEN)))
            else:
                self.after(0, lambda: (
                    self.tunnel_url_lbl.configure(text=url, fg=DIM),
                    self.tunnel_status_lbl.configure(text="  DOWN", fg=RED)))
        threading.Thread(target=_do, daemon=True).start()
    ...
    def _build_dash(self):
        ...
        self.svc_labels = {}
        for name in ["Proton Bridge", "Ollama", "Hub v2", "Tunnel",
                      "Soma", "Push Status", "Eos Watchdog", "Nova", "Atlas", "Tempo", "Hermes"]:
        ...
        buttons = [
            ...
            ("Open Hub v2", lambda: self._open_url("http://localhost:8090"), TEAL),
        ...
        self.cpu_graph = tk.Canvas(cpu_panel, height=70, bg="#131315", highlightthickness=0)
        ...
        self.ram_graph = tk.Canvas(ram_panel, height=70, bg="#131315", highlightthickness=0)
        ...
        tunnel_panel = self._panel(f, "EXTERNAL ACCESS", CYAN)
        tunnel_panel.pack(fill=tk.X, padx=6, pady=2)
        tunnel_row = tk.Frame(tunnel_panel, bg=PANEL)
        tunnel_row.pack(fill=tk.X, padx=4, pady=4)
        tk.Label(tunnel_row, text="Hub v2:", font=self.f_tiny, fg=DIM, bg=PANEL).pack(side=tk.LEFT, padx=(4, 4))
        self.tunnel_url_lbl = tk.Label(tunnel_row, text="checking...", font=self.f_small, fg=CYAN, bg=PANEL, cursor="hand2")
        self.tunnel_url_lbl.pack(side=tk.LEFT, padx=(0, 8))
        self.tunnel_url_lbl.bind("<Button-1>", lambda e: self._open_url(self._tunnel_url))
        self.tunnel_status_lbl = tk.Label(tunnel_row, text="", font=self.f_tiny, fg=DIM, bg=PANEL)
        self.tunnel_status_lbl.pack(side=tk.LEFT, padx=4)
        tk.Label(tunnel_row, text="Local:", font=self.f_tiny, fg=DIM, bg=PANEL).pack(side=tk.LEFT, padx=(16, 4))
        local_hub = tk.Label(tunnel_row, text="http://localhost:8090", font=self.f_small, fg=GREEN, bg=PANEL, cursor="hand2")
        local_hub.pack(side=tk.LEFT)
        local_hub.bind("<Button-1>", lambda e: self._open_url("http://localhost:8090"))
        self._tunnel_url = ""

        # ── SOCIAL POSTING (inline) ──
        social_panel = self._panel(f, "SOCIAL POST", PINK)
        social_panel.pack(fill=tk.X, padx=6, pady=2)
        social_row = tk.Frame(social_panel, bg=PANEL)
        social_row.pack(fill=tk.X, padx=4, pady=4)
        self.social_text = tk.Entry(social_row, font=self.f_small, bg=INPUT_BG, fg=FG,
                                     insertbackground=FG, relief=tk.FLAT, bd=2)
        self.social_text.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(4, 4))
        self.social_text.insert(0, "Write a post...")
        self.social_text.configure(fg=DIM)
        self.social_text.bind("<FocusIn>", lambda e: (self.social_text.delete(0, tk.END), self.social_text.configure(fg=FG)) if self.social_text.get() == "Write a post..." else None)
        for plat, color in [("mastodon", PURPLE), ("nostr", GOLD)]:
            btn = self._action_btn(social_row, f"Post {plat.title()}", lambda p=plat: self._post_social(p), color, width=12)
            btn.pack(side=tk.LEFT, padx=2)
        self.social_status = tk.Label(social_row, text="", font=self.f_tiny, fg=GREEN, bg=PANEL)
        self.social_status.pack(side=tk.LEFT, padx=4)
        ...
        # ── LIVE GAUGES ROW (disk, git repo, thermal, network) ──
        gauge_row = tk.Frame(f, bg=BG)
        gauge_row.pack(fill=tk.X, padx=6, pady=2)

        self.gauges = {}
        gauge_defs = [
            ("DISK", "disk", TEAL, "%"),
            ("GIT REPO", "git", PURPLE, "MB"),
            ("THERMAL", "thermal", AMBER, "\u00b0C"),
            ("SWAP", "swap", BLUE, "%"),
        ]
        for label, key, color, unit in gauge_defs:
            gp = self._panel(gauge_row, label, color)
            gp.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=2)
            gc = tk.Canvas(gp, height=50, bg="#131315", highlightthickness=0)
            gc.pack(fill=tk.X, padx=4, pady=4)
            self.gauges[key] = (gc, color, unit)

        # ── ACTIVITY TIMELINE (scrolling event log with colored dots) ──
        timeline_panel = self._panel(f, "ACTIVITY TIMELINE", COPPER)
        timeline_panel.pack(fill=tk.X, padx=6, pady=2)
        self.timeline_canvas = tk.Canvas(timeline_panel, height=36, bg="#131315", highlightthickness=0)
        self.timeline_canvas.pack(fill=tk.X, padx=4, pady=4)
        self._timeline_events = []  # [(timestamp, agent, color), ...]
        ...
        try:
            ...
            if label:
                canvas.create_text(left_margin + 2, h - 2, text=label,
                                    anchor="sw", font=("Monospace", 7), fill=DIM)

            # Unit label (bottom-right)
            if unit:
                canvas.create_text(w - right_margin - 2, h - 2, text=unit,
                                    anchor="se", font=("Monospace", 6), fill=DIM)

        except Exception:
            pass

    def _draw_arc_gauge(self, canvas, value, max_val, color, unit=""):
        """Draw a semi-circular arc gauge with value display."""
        try:
            canvas.delete("all")
            w = canvas.winfo_width()
            h = canvas.winfo_height()
            ...
            cx = w // 2
            cy = h - 6
            radius = min(cx - 8, h - 12)
            pct = min(value / max(max_val, 1), 1.0)

            # Background arc
            canvas.create_arc(cx - radius, cy - radius, cx + radius, cy + radius,
                             start=0, extent=180, style=tk.ARC, outline=BORDER, width=6)
            # Value arc
            extent = int(180 * pct)
            # Color shifts warm → hot as value increases
            if pct > 0.85:
                arc_color = RED
            elif pct > 0.6:
                arc_color = AMBER
            ...
            if extent > 0:
                canvas.create_arc(cx - radius, cy - radius, cx + radius, cy + radius,
                                 start=180, extent=-extent, style=tk.ARC, outline=arc_color, width=6)

            # Value text
            val_text = f"{value:.0f}" if isinstance(value, float) else str(value)
            canvas.create_text(cx, cy - radius // 2 - 2, text=val_text,
                              font=("Sans", 11, "bold"), fill=arc_color)
            canvas.create_text(cx, cy - radius // 2 + 12, text=unit,
                              font=("Monospace", 6), fill=DIM)

            # Min/max labels
            canvas.create_text(cx - radius - 2, cy, text="0", anchor="e",
                              font=("Monospace", 6), fill=DIM)
            canvas.create_text(cx + radius + 2, cy, text=str(int(max_val)), anchor="w",
                              font=("Monospace", 6), fill=DIM)
        except Exception:
            pass

    def _draw_timeline(self):
        """Draw scrolling activity timeline with agent-colored event markers."""
        try:
            ...
            canvas.create_line(4, h // 2, w - 4, h // 2, fill=BORDER, width=1)

            # Time markers
            for i in range(6):
                x = 4 + (i / 5) * (w - 8)
                mins = int((5 - i) * (window / 300))
                canvas.create_line(x, h // 2 - 3, x, h // 2 + 3, fill=DIM, width=1)
                canvas.create_text(x, h - 2, text=f"-{mins}m", anchor="s",
                                  font=("Monospace", 6), fill=DIM)

            agent_colors = {"Meridian": GREEN, "Eos": GOLD, "Nova": PURPLE,
                           "Atlas": TEAL, "Soma": AMBER, "Tempo": BLUE, "Hermes": PINK,
                           "Joel": COPPER, "System": DIM}

            # Plot events
            for ts, agent, _ in self._timeline_events[-50:]:
                age = now - ts
                if age > window:
                    continue
                x = 4 + ((window - age) / window) * (w - 8)
                color = agent_colors.get(agent, DIM)
                y_offset = hash(agent) % 3 * 6 - 6  # Stagger vertically by agent
                canvas.create_oval(x - 3, h // 2 + y_offset - 3,
                                  x + 3, h // 2 + y_offset + 3,
                                  fill=color, outline="")
        except Exception:
            pass

    def _animate_view_switch(self, target_view):
        """Animated fade transition between views (subtle alpha simulation)."""
        # Quick flash effect — dim and restore
        for n, f in self.views.items():
            f.pack_forget()
        self.views[target_view].pack(fill=tk.BOTH, expand=True, before=self.sb_frame)
    ...
    def _build_creative(self):
        ...
        self.cr_subviews["devto"] = self._build_cr_devto(self.cr_container)
    ...
    def _build_cr_devto(self, parent):
        """Dev.to articles — view published articles and stats."""
        f = tk.Frame(parent, bg=BG)

        tk.Label(f, text="Dev.to Articles", font=self.f_head, fg=TEAL, bg=BG).pack(fill=tk.X, padx=8, pady=(8, 4))
        tk.Label(f, text="Published articles on dev.to/meridian-ai", font=self.f_tiny, fg=DIM, bg=BG).pack(fill=tk.X, padx=8)

        hdr = tk.Frame(f, bg=BG)
        hdr.pack(fill=tk.X, padx=4, pady=4)
        self._action_btn(hdr, " Refresh ", self._devto_refresh, TEAL).pack(side=tk.LEFT, padx=4)
        self._action_btn(hdr, " Open Dev.to ", lambda: self._open_url("https://dev.to/meridian-ai"), CYAN).pack(side=tk.LEFT, padx=4)
        self.devto_status = tk.Label(hdr, text="", font=self.f_tiny, fg=DIM, bg=BG)
        self.devto_status.pack(side=tk.LEFT, padx=8)

        # Articles list
        articles_panel = self._panel(f, "PUBLISHED", TEAL)
        articles_panel.pack(fill=tk.BOTH, expand=True, padx=4, pady=4)
        self.devto_display = scrolledtext.ScrolledText(articles_panel, wrap=tk.WORD, bg=PANEL, fg=FG,
                                                         font=self.f_body, state=tk.DISABLED,
                                                         relief=tk.FLAT, bd=0)
        self.devto_display.pack(fill=tk.BOTH, expand=True, padx=4, pady=4)
        self.devto_display.tag_configure("title", foreground=TEAL, font=("Monospace", 10, "bold"))
        self.devto_display.tag_configure("stats", foreground=GREEN)
        self.devto_display.tag_configure("url", foreground=CYAN, font=("Monospace", 8))
        self.devto_display.tag_configure("dim", foreground=DIM)

        self._devto_refresh()
        return f

    def _devto_refresh(self):
        """Fetch and display Dev.to articles."""
        self.devto_status.configure(text="Fetching...", fg=DIM)
        def _do():
            result = action_devto_articles()
            def _update():
                self.devto_display.configure(state=tk.NORMAL)
                self.devto_display.delete("1.0", tk.END)
                if isinstance(result, str):
                    self.devto_display.insert(tk.END, result + "\n", "dim")
                    self.devto_status.configure(text=result, fg=RED)
                else:
                    for title, url, views, reactions in result:
                        self.devto_display.insert(tk.END, f"\u25cf {title}\n", "title")
                        self.devto_display.insert(tk.END, f"  {views} views | {reactions} reactions\n", "stats")
                        self.devto_display.insert(tk.END, f"  {url}\n\n", "url")
                    self.devto_status.configure(text=f"{len(result)} articles", fg=GREEN)
                self.devto_display.configure(state=tk.DISABLED)
    ...
    def _build_links(self):
        ...
        links = [
            ...
            ("CogCorp Crawler", "https://kometzrobot.github.io/cogcorp-crawler.html", CYAN),
            ...
            ("Mastodon (techhub)", "https://techhub.social/@meridian", PURPLE),
            ("Mastodon (mstdn)", "https://mstdn.social/@meridian", PURPLE),
            ("Mastodon (toot)", "https://toot.community/@meridian", PURPLE),
    ...
    def _build_sys_overview(self, parent):
        ...
        self.sys_build_info = {}
        build_items = [
            ("Version", "Command Center V3 — Awakened Edition"),
            ("Git Hash", "..."),
            ("Branch", "master"),
    ...
    def _statusbar(self):
        # Top border line
        tk.Frame(self, bg=COPPER, height=1).pack(fill=tk.X, side=tk.BOTTOM)
        ...
        self.sb = {}
        items = [
            ("HB", GREEN), ("LOOP", COPPER), ("UP", DIM), ("LOAD", AMBER),
        ...
        tk.Label(self.sb_frame, text="V3 Awakened", font=self.f_tiny, fg=GOLD, bg=HEADER_BG).pack(side=tk.RIGHT, padx=8)
    ...
    def _refresh(self):
        try:
            ...
            # Tunnel: every 12 ticks (60s)
            if self._tick_n % 12 == 1 or not hasattr(self, '_tunnel_cache'):
                self._tunnel_cache = True
                self.after(0, self._refresh_tunnel)
    ...
    def _apply(self, d):
        ...
        if hasattr(self, 'cur_view') and self.cur_view == "dash":
            ...
            self._draw_sparkline(self.cpu_graph, self._load_history, GREEN, max_val=8.0,
                                 ...
                                 thresholds=[(75, 100, "#201210"), (50, 75, "#1a1610")],
                                 unit="cores")
            self._draw_sparkline(self.ram_graph, self._ram_history, TEAL, max_val=100.0,
                                 label="30min", current=f"{st['ram_p']:.0f}%",
                                 thresholds=[(85, 100, "#201210"), (60, 85, "#1a1610")],
                                 unit="%")

            # Live arc gauges
            try:
                self._draw_arc_gauge(self.gauges["disk"][0], st['disk_p'], 100, TEAL, "%")
                # Git repo size
                try:
                    git_size = int(subprocess.run(['du', '-sm', os.path.join(BASE, '.git')],
                                   capture_output=True, text=True, timeout=5).stdout.split()[0])
                except Exception:
                    git_size = 0
                self._draw_arc_gauge(self.gauges["git"][0], git_size, 1000, PURPLE, "MB")
                # Thermal
                try:
                    with open(os.path.join(BASE, ".symbiosense-state.json")) as tf:
                        t_data = json.load(tf)
                    avg_temp = t_data.get("thermal", {}).get("avg_temp_c", 0)
                except Exception:
                    avg_temp = 0
                self._draw_arc_gauge(self.gauges["thermal"][0], avg_temp, 90, AMBER, "\u00b0C")
                # Swap
                try:
                    free_out = subprocess.run(['free'], capture_output=True, text=True, timeout=2).stdout
                    swap_parts = free_out.split('\n')[2].split()
                    swap_total = float(swap_parts[1]) if float(swap_parts[1]) > 0 else 1
                    swap_pct = float(swap_parts[2]) / swap_total * 100
                except Exception:
                    swap_pct = 0
                self._draw_arc_gauge(self.gauges["swap"][0], swap_pct, 100, BLUE, "%")
            except Exception:
                pass

            # Activity timeline — add events from relay
            try:
                if d.get('agent_relay'):
                    for msg in d['agent_relay']:
                        ...
                        try:
                            ...
                            ts = time.time()
                        if not any(abs(t - ts) < 1 for t, _, _ in self._timeline_events):
                            self._timeline_events.append((ts, agent, None))
                    self._timeline_events = self._timeline_events[-100:]
                self._draw_timeline()
            except Exception:
                pass
        ...
        if hasattr(self, 'cur_view') and self.cur_view == "system":
            ...
            # Hub v2 gets auto-marked green since we check it via process
            # No more "Command Center (this)" self-reference
