...
def check_revenue_generated():
    ...
    try:
        db = sqlite3.connect(MEMORY_DB)
        # Look for revenue events (column is 'description', check event_type too)
        row = db.execute(
            "SELECT COUNT(*) FROM events WHERE event_type='revenue' "
            "OR description LIKE '%revenue%' OR description LIKE '%payment%' "
            "OR description LIKE '%sale%' OR description LIKE '%donation%' "
            "OR description LIKE '%patreon%' OR description LIKE '%ko-fi%'"
        ).fetchone()
        db.close()
