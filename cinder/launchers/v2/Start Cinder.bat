@echo off
REM CINDER_LAUNCHER_VERSION=2   bump + republish manifest to push patches
...
REM
REM LAUNCHER_VERSION must match the comment marker on line 2. Bumping it on
REM every patch is what stops self-update from looping (manifest version >
REM local version is the only condition that triggers a re-download).
...
if not "%CINDER_NO_SELF_UPDATE%"=="1" (
    ...
    powershell -nologo -ExecutionPolicy Bypass -command "try { $m = Invoke-RestMethod -Uri 'https://kometzrobot.github.io/cinder/launchers/manifest.json' -TimeoutSec 5; if($m.windows_version -gt %LAUNCHER_VERSION%){ Invoke-WebRequest -Uri $m.windows_url -OutFile '%~dp0Start Cinder.bat.new' -TimeoutSec 30 -UseBasicParsing; $h = (Get-FileHash -Path '%~dp0Start Cinder.bat.new' -Algorithm SHA256).Hash.ToLower(); if($h -ne $m.windows_sha256.ToLower()){ Remove-Item '%~dp0Start Cinder.bat.new' -Force -ErrorAction SilentlyContinue } } } catch {}" >nul 2>&1
...
REM Only set WE_STARTED_OLLAMA when WE actually spawned the process — without
REM this guard, the cleanup at the end would kill a pre-existing ollama left
REM by another Cinder window or the buyer's other apps. Same orphan-kill fix
REM Mac and Linux got in Loop 9585.
...
REM Kill only the ollama WE started — leave a pre-existing instance on 11436
REM alone (it may have been spawned by another Cinder window or left running
REM intentionally). System ollamas on 11434 are never touched.
if defined WE_STARTED_OLLAMA (
