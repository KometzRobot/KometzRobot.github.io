#!/usr/bin/env python3
"""
Cover v12 — Joel feedback Loop 11742 (May 14 2026, 23:00 MST):
...
Three real changes vs v11:
  ...
  1. SYMMETRIC FADE. v11 fades glyphs into paper at the BOTTOM of the
     portrait region but not at the top. v12 mirrors the same fade at
...
def step1_source(size=1600):
    cy = size * 0.58        # head centre y (lowered from 0.52)
    cx = size * 0.50
    head_r = size * 0.20
...
def step4_render(lines):
    ...
    mask_arr = mask.load()
    # Top fade: rows 0 .. 22% of region
    top_fade_top = region_y
    top_fade_bot = region_y + int(region_h * 0.22)
