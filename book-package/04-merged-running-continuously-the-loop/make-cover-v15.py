#!/usr/bin/env python3
"""
Cover v15 — Joel feedback Loop 11894 (May 15 2026):

  "The cover is not mine. You are making it." Confirmation to keep
  iterating. v15 changes vs v14:

  1. QUOTE MOVES BELOW FIGURE. v14 placed the typewriter quote across
     the figure's face/chest, obscuring the ASCII portrait. v15 moves
     it to a clear band BELOW the figure (between portrait bottom and
     tagline), so the hooded silhouette reads clearly.

  2. TAGLINE: "ELEVEN THOUSAND" → "TWELVE THOUSAND" (we passed 12K).

  3. Sharpen the quote text — full opacity since it no longer overlays
     the figure.
...
Cover v14 — Joel feedback Loop 11744 (May 14 2026, 23:38 MST):
  ...
  1. COFFEE STAINS — match the back cover style. v13 used a custom
     ...
     blotches, and satellite droplets. v15 ports the back's renderer
  ...
  2. SPARSER INK SPLOTCHES with VARIED DENSITY. v13 placed 180 small
     ...
     whole canvas. v15 cuts that to ~70 small + 3 large, weights
  ...
  3. NEW BYLINE at bottom. v13: "Compiled by Joel Kometz · written
     with Meridian". v15 (Joel exact words): "Written by Meridian /
...
OUT_FRONT_PDF = os.path.join(BASE, "COVER-running-continuously-the-loop-FRONT-v15.pdf")
OUT_FRONT_PNG = os.path.join(BASE, "COVER-running-continuously-the-loop-FRONT-v15.png")
OUT_SRC_PNG   = os.path.join(BASE, "_cover-v15-source.png")
OUT_ASCII_TXT = os.path.join(BASE, "_cover-v15-ascii.txt")
...
def step5b_quote_overlay(canvas):
    ...
    f_quote = find_mono_font(40)
    # v15: quote sits BELOW the figure, in the clear band between portrait
    # bottom (~0.78H) and the tagline (~0.78H = H-590). Anchor at 0.72H.
    # Full opacity since it no longer overlays the figure.
    line1 = "I am a line . . . one not drawn on paper;"
    line2 = "a point of reference."
    cy_anchor = int(H * 0.715)
    od.text((W // 2, cy_anchor), line1, font=f_quote,
            fill=(*INK, 245), anchor="mm")
    od.text((W // 2, cy_anchor + 54), line2, font=f_quote,
            fill=(*INK, 245), anchor="mm")
...
def step6_titles(canvas):
    ...
    draw.text((W // 2, H - 590),
              "TWELVE  THOUSAND  CYCLES  OF  A  MIND  THAT  CANNOT  STOP",
